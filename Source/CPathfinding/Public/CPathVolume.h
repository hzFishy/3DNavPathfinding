...
class CPATHFINDING_API ACPathVolume : public AActor
{
	...
	----------------------------------------------------------------------------*/
public:
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath", meta = (EditCondition = "bGenerationStarted==false"))
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Components", meta = (EditCondition = "bGenerationStarted==false"))
	TObjectPtr<UBoxComponent> VolumeBox;

	/** Spports Capsule, sphere and box. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath", meta = (EditCondition = "bGenerationStarted==false"))
	TEnumAsByte<EAgentShape> AgentShape;

	/** In case of a box, this is X and Y extent. Z and Y should be the same, since the actual agent will most likely rotate. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath", meta = (EditCondition = "bGenerationStarted==false", ClampMin = "0", UIMin = "0"))
	float AgentRadius;

	/** In case of a box, this is Z extent. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath", meta = (EditCondition = "bGenerationStarted==false && AgentShape!=EAgentShape::Sphere", ClampMin = "0", UIMin = "0"))
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath", meta = (EditCondition = "bGenerationStarted==false", ClampMin = "0.1", UIMin = "0.1"))
	float VoxelSize;
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath", meta = (EditCondition = "bGenerationStarted==false", ClampMin = "0.01", UIMin = "0.01", ClampMax = "30", UIMax = "30"))
	float DynamicObstaclesUpdateRate;
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath", meta = (EditCondition = "bGenerationStarted==false", ClampMin = "0", ClampMax = "3", UIMin = "0", UIMax = "3"))
	int32 OctreeDepth;
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath")
	bool bOverwriteMaxGenerationThreads;
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath", meta = (EditCondition = "bGenerationStarted==false && bOverwriteMaxGenerationThreads==true", ClampMin = "0", ClampMax = "31", UIMin = "0", UIMax = "31"))
	int32 MaxGenerationThreads;
	...
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "CPath|Info")
	bool bInitialGenerationFinished;
	...
	bool bPerformBenchmarkAfterGeneration;

	/** Benchmark duration in seconds */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath|Benchmark")
	float BenchmarkDurationSeconds;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath|Benchmark")
	int32 BenchmarkFindPathUserData;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath|Benchmark")
	float BenchmarkFindPathTimeLimit;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath|Benchmark")
	FString BenchmarkName;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath|Benchmark")
	bool bSaveBenchmarkResultToFile;
	
	/** if sample size of successful searches is too low, don't save the results to file */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath|Benchmark", meta = (EditCondition = "bSaveBenchmarkResultToFile==true"))
	bool bSaveBenchmarksWithUnreliableResults;
	
	/** Not used for now, async benchmark does not provide reliable results */
	//UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "CPath|Benchmark")
	bool bIsAsyncBenchmark;
...
protected:
	...
	UPROPERTY()
	TObjectPtr<ACPathCore> CoreInstance;
...
public:
	...
	bool FindPathAsync(UObject* CallingObject, const FName& InFunctionName, FVector Start, FVector End,
		uint32 SmoothingPasses = 2, int32 UserData = 0, float TimeLimit = 0.15f, bool RequestRawPath = false, bool RequestUserPath = true);
	
	/** Same as above, just using the FCPathRequest structure to pass parameters */
	bool FindPathAsync(const FCPathRequest& Request);
	...
	FCPathResult FindPathSynchronous(FVector Start, FVector End, uint32 SmoothingPasses = 2, int32 UserData = 0, float TimeLimit = 0.002f, 
	...
	void FindPathSynchronous(TEnumAsByte<BranchFailSuccessEnum>& Branches, TArray<FCPathNode>& Path, TEnumAsByte<ECPathfindingFailReason>& FailReason,
		FVector Start, FVector End, int32 SmoothingPasses = 2, int32 UserData = 0, float TimeLimit = 0.002f);
...
protected:
	...
	int32 WorldLocationToIndex(FVector WorldLocation) const;
